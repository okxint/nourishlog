    ...
    <div className="px-5 lg:px-8 pt-10 lg:pt-8 fade-in">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        {dateRange === 'week' && (
          <div className="flex items-center gap-1 ml-2">
            ...
            <span className="text-xs font-semibold min-w-[90px] text-center" style={{ color: 'var(--text-secondary)', fontFamily: 'Space Grotesk' }}>
      ...
      {showFilters && (
        <div className="glass-card rounded-xl p-4 mb-4 flex flex-col sm:flex-row gap-3 slide-up">
          <div className="flex-1">
            <label className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-1 block" style={{ fontFamily: 'Space Grotesk' }}>Meal Type</label>
          ...
          <div className="flex-1">
            <label className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-1 block" style={{ fontFamily: 'Space Grotesk' }}>Search Food</label>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-5">
        <div className="glass-card rounded-xl p-3 text-center">
          <p className="text-xl font-extrabold text-[var(--accent)]" style={{ fontFamily: 'Sora' }}>{totals.cal.toLocaleString()}</p>
          <p className="text-[9px] text-[var(--text-muted)] mt-0.5 uppercase font-semibold" style={{ fontFamily: 'Space Grotesk' }}>Total kcal</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          <p className="text-xl font-extrabold" style={{ fontFamily: 'Sora' }}>{avgCalories.toLocaleString()}</p>
          <p className="text-[9px] text-[var(--text-muted)] mt-0.5 uppercase font-semibold" style={{ fontFamily: 'Space Grotesk' }}>Daily Avg</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          <p className="text-xl font-extrabold" style={{ fontFamily: 'Sora' }}>{filteredEntries.length}</p>
          <p className="text-[9px] text-[var(--text-muted)] mt-0.5 uppercase font-semibold" style={{ fontFamily: 'Space Grotesk' }}>Meals</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          <p className="text-xl font-extrabold" style={{ fontFamily: 'Sora' }}>{uniqueDays}</p>
          <p className="text-[9px] text-[var(--text-muted)] mt-0.5 uppercase font-semibold" style={{ fontFamily: 'Space Grotesk' }}>Days</p>
        </div>
      </div>
      ...
      {dateRange !== 'today' && chartData.length > 0 && (
        <div className="flex flex-col lg:flex-row gap-4 mb-4">
          <div className="flex-[2] glass-card rounded-xl p-4">
            <h3 className="text-sm font-bold mb-3" style={{ fontFamily: 'Sora' }}>
              Calories {mealFilter !== 'all' ? `(${mealFilter})` : ''} {foodSearch ? `matching "${foodSearch}"` : ''}
            </h3>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} barCategoryGap="15%">
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 10, fontFamily: 'Space Grotesk' }} />
                <YAxis hide />
                <Tooltip
                  contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 11, fontFamily: 'DM Sans' }}
          ...
          <div className="flex-1 glass-card rounded-xl p-4">
            <h3 className="text-sm font-bold mb-3" style={{ fontFamily: 'Sora' }}>Macro Split</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-5">
        {[
          ...
          <div key={s.label} className="glass-card rounded-xl p-3">
            ...
            <p className="text-[9px] text-[var(--text-muted)] uppercase tracking-wide" style={{ fontFamily: 'Space Grotesk' }}>{s.label}</p>
            <p className="text-sm font-bold truncate" style={{ fontFamily: 'Sora' }}>{s.value}</p>
      ...
      <div className="mb-8">
        <button onClick={() => setShowHistory(!showHistory)}
          className="flex items-center gap-2 mb-3 text-sm font-bold hover:text-[var(--accent)] transition-colors" style={{ fontFamily: 'Sora' }}>
        ...
        {showHistory && (
          <div className="glass-card rounded-xl overflow-hidden">
            {/* Table header */}
            <div className="hidden lg:grid grid-cols-12 gap-2 px-4 py-2 text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wide border-b border-[var(--border)]" style={{ fontFamily: 'Space Grotesk' }}>
            ...
            {historyByDate.length === 0 ? (
              ...
              historyByDate.map(([date, entries]) => (
                <div key={date}>
                  {/* Date header (mobile) */}
                  <div className="lg:hidden px-4 py-2 text-[11px] font-bold border-b border-[var(--border)]" style={{ background: 'var(--bg-elevated)', fontFamily: 'Sora' }}>
